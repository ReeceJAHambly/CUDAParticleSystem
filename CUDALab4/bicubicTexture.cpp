...
bool wind = false; //Bool used for wind 
bool gravity = false; //Bool used for gravity
...
ParticleSystem *d = new ParticleSystem(numOfParticles);
...
void display()
{
	...
	size_t num_bytes;
	float xPos[numOfParticles] = { 0 };
	float yPos[numOfParticles] = { 0 };
	for (int i = 0; i < d->particleList.size(); i++) {
		xPos[i] = d->particleList[i].x;
		yPos[i] = d->particleList[i].y;
	}
	checkCudaErrors(cudaGraphicsResourceGetMappedPointer((void **)&d_output, &num_bytes,cuda_pbo_resource));
	render(imageWidth, imageHeight,blockSize, gridSize,d_output,xPos,yPos,wind,gravity);
	checkCudaErrors(cudaGraphicsUnmapResources(1, &cuda_pbo_resource, 0));

	for (int i = 0; i < d->particleList.size(); i++) {
		d->particleList[i].x = xPos[i];
		d->particleList[i].y = yPos[i];
	}
